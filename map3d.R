...
library(rgl)
library(dplyr)
library(scales)  # For adjusting brightness
...
# Define Base Colors for Regions (No Lightening)
...
# 1️⃣ Draw Region Boundaries in 3D
for (i in 1:nrow(map_data)) {
  region_coords <- st_coordinates(st_cast(map_data$geometry[i], "MULTILINESTRING"))
  lines3d(region_coords[,1] - center_x, region_coords[,2] - center_y, rep(0, nrow(region_coords)), col = "gray", lwd = 0.5)  
}

# 2️⃣ Extrude Each Region into a 3D Prism with Proper Coloring
for (i in 1:nrow(map_data)) {
  ...
  if (!is.null(region_poly)) {
    ...
    polygon3d(poly_coords[,1] - center_x, poly_coords[,2] - center_y, rep(0, nrow(poly_coords)), col = map_data$region_color[i], alpha = 0.8)
    
    # Top of the extruded region
    polygon3d(poly_coords[,1] - center_x, poly_coords[,2] - center_y, rep(map_data$height[i], nrow(poly_coords)), col = map_data$region_color[i], alpha = 0.9)
    ...
    for (j in 1:(nrow(poly_coords) - 1)) {
      ...
      quads3d(x_vals, y_vals, z_vals, col = map_data$region_color[i], alpha = 0.8)
    }
  }
}
