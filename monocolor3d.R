...
# Generate a Color Gradient (Dark = High, Light = Low)
# Normalize heights and replace NAs with 0
height_scaled <- map_data$height / max(map_data$height, na.rm = TRUE)  
height_scaled[is.na(height_scaled)] <- 0  # Replace NAs with 0

# Ensure values stay within the valid [0,1] range
height_scaled <- pmax(0, pmin(1, height_scaled))  

# Generate a blue color gradient where higher population = darker blue
color_map <- rgb(0, 0, height_scaled, alpha = 0.8)


# Open 3D Device with Rotation Limits
open3d()
par3d(windowRect = c(50, 50, 1000, 800))  # Adjusts window size
par3d(userMatrix = rotationMatrix(0, 1, 0, 0))  # Locks the vertical flip
par3d(ignoreExtent = TRUE)  # Ensures it doesn't rotate backwards

# Draw Region Boundaries in 3D (Base Layer)
for (i in 1:nrow(map_data)) {
  region_coords <- st_coordinates(st_cast(map_data$geometry[i], "MULTILINESTRING"))  # Convert to lines
  lines3d(region_coords[,1], region_coords[,2], rep(0, nrow(region_coords)), col = "black", lwd = 0.7)  # Base map
}

# Extrude Each Region into a 3D Prism with Color Intensity Based on Height
for (i in 1:nrow(map_data)) {
  region_poly <- st_cast(map_data$geometry[i], "POLYGON")  # Convert to polygon format
  if (!is.null(region_poly)) {
    poly_coords <- st_coordinates(region_poly)  # Extract polygon coordinates
    
    # Base of the region at z=0
    polygon3d(poly_coords[,1], poly_coords[,2], rep(0, nrow(poly_coords)), col = color_map[i], alpha = 0.5)
    
    # Top of the extruded region
    polygon3d(poly_coords[,1], poly_coords[,2], rep(map_data$height[i], nrow(poly_coords)), col = color_map[i], alpha = 0.7)
    ...
    for (j in 1:(nrow(poly_coords) - 1)) {
      ...
      quads3d(x_vals, y_vals, z_vals, col = color_map[i], alpha = 0.6)
    }
  }
}

#  Adjust View for Better Interaction
